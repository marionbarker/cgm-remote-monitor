  ...
  function drawChart (day, data, options) {
    ...
    for (var dt = moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      if (options.basal) {
        ...
        // modify next two lines code for my personal use with Omnipod
        // . baseBasalInsulin += basalvalue.basal * 5 / 60; // 5 minutes part
        // . var tempPart = (basalvalue.tempbasal - basalvalue.basal) * 5 / 60;
        var tempSchBasal = basalvalue.basal * 5 / 60; // 5 minutes worth, ignore floor
        baseBasalInsulin += tempSchBasal;
        var tempFive = (basalvalue.tempbasal * 5 / 60 ); // ideal value, no floor
        var tempPart = tempFive - tempSchBasal; 
        if (tempPart > 0) {
          tempPart = Math.floor(20.0 * tempFive) / 20.0 - tempSchBasal; // quantize Pod
          positiveTemps += tempPart;
          data.netBasalPositive[hournow] += tempPart;
        }
        if (tempPart < 0) {
          tempPart = tempFive - tempSchBasal;  // go with full neg TB
