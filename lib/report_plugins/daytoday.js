  ...
  function drawChart (day, data, options) {
    ...
    for (var dt = moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      if (options.basal) {
        ...
        baseBasalInsulin += floor(20.0 * basalvalue.basal * 5 / 60) / 20.0; // 5 minutes worth, floor to 0.05 u
        var tempPart = floor(20.0 * (basalvalue.tempbasal - basalvalue.basal) * 5 / 60) / 20.0; 
        if (tempPart > 0) {
