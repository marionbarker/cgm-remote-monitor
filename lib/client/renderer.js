...
function init (client, d3) {
  ...
  function prepareArc (treatment, radius) {
    ...
    if (treatment.insulin > 0) {
      var insulin_string = treatment.insulin.toFixed(2);

      var enteredBy = '' + treatment.enteredBy;

      if (treatment.insulin < 1 ) { 
        // remove leading zeros to minimize overlap with adjacent boluses
        //insulin_string = insulin_string.replace(/^0/, "");
      }

      arc_data[3].element = insulin_string;
    }
